...
const dlcTemplateOrganPlayerKeyPressedOnlyStrategies = {
    // 示例器官-按键激活
    'golden_age:example_organ_key_pressed': function (event, organ) {
        // 获取玩家
        ...
        // 添加冷却时间
        // 这里不需要额外做检测，本体已经做好了
        player.addItemCooldown('golden_age:example_organ_key_pressed', 20 * 15)
    },
    
    
	'golden_age:machine': function (event, organ) {
		// 获取物品
		let item = event.item
		// 获取玩家
		let player = event.player
		...
		if (count > 10000) {
			player.give(Item.of('golden_age:neutronium_ingot'))
			updateResourceCount(player, count - 10000)}
