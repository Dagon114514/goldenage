...
const dlcTemplateOrganPlayerDamageStrategies = {
	// 示例器官-伤害
	'golden_age:example_organ_damage': function (event, organ, data) {
		// 造成的伤害变为110%
		event.amount *= 1.1
	},
}
...
const dlcTemplateOrganPlayerDamageOnlyStrategies = {
	// 示例器官-唯一伤害
	'golden_age:example_organ_damage_only': function (event, organ, data) {
		// 如果仍在冷却中，则不执行
	...
	'golden_age:infinity_sword_organ': function (event, organ, data) {
		let player = event.source.player
		let entity = event.entity
		let entityList = getLivingWithinRadius(entity.getLevel(), entity.position(), 3)
		entityList.forEach(entity => {
